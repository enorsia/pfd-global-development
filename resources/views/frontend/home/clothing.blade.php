<!---- Looking For Section -->
<div id="looking">
    <div class="container">
        <div class="row justify-content-center">
            <div class="col-lg-7 col-md-10 col-12 text-center">
                <div class="looking_body">
                    <div class="looking_title">
                        <h2>Explore Our Clothing</h2>
                        <span class="global_underline"><i class="fa-regular fa-snowflake"></i></span>
                    </div>
                    <div class="looking_text">
                        <p>
                            PFD GLOBAL is one of the
                            <span>leading clothing manufacturers</span>, producing
                            garments of the highest quality. We offer a full scale
                            manufacturing solution, proudly supporting fashion startups,
                            small clothing brands, SMEs and emerging designers.
                        </p>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>
<!---- End Looking For Section -->
<!--  Tech Pack Section-->
<div id="tech">
    <div class="container">
        <div class="row justify-content-center">
            <div class="col-xl-10 col-lg-12 col-12">
                <div class="tech_body one">
                    <div class="tech_text">
                        <h2>Men's Fashion</h2>
                        <p>
                            Discover the perfect blend of style and comfort with our
                            extensive men's fashion collection. Whether you're dressing
                            for a formal occasion, a casual day out, or a cozy night in,
                            our selection has everything you need. From versatile Chinos
                            to stylish Jeans, we offer a range of bottoms that cater to
                            every preference. Stay comfortable and on-trend with our
                            Joggers and Trousers, or embrace the ultimate in relaxation
                            with our Loungewear and Nightwear options.
                        </p>
                        <p>
                            Our collection also features Polo Shirts, Shirts, and T-Shirts
                            that make it easy to elevate your look for any event. For
                            those cooler days, layer up with our chic Coats, Jackets,
                            Sweatshirts, and Hoodies. Plus, our Co-ords and Multipacks
                            make styling effortless with coordinated outfits at great
                            value.
                        </p>
                    </div>

                    <div class="tech_img">
                        @if (Str::isMobile())
                            <img src="https://imagedelivery.net/z7kNRC7XttEX5-KXLRrx3A/7d54637a-b68c-4fa2-411e-97740553c600/w=205" alt="MEN" title="MEN" />
                        @else
                            <img src="https://imagedelivery.net/z7kNRC7XttEX5-KXLRrx3A/7d54637a-b68c-4fa2-411e-97740553c600/w=350" alt="MEN" title="MEN" />
                        @endif
                    </div>
                </div>
                <div class="tech_body two">
                    <div class="tech_img">
                        @if (Str::isMobile())
                            <img src="https://imagedelivery.net/z7kNRC7XttEX5-KXLRrx3A/6e595f57-1dbe-4679-ac86-63fdeb781a00/w=205" alt="WOMEN" />
                        @else
                            <img src="https://imagedelivery.net/z7kNRC7XttEX5-KXLRrx3A/6e595f57-1dbe-4679-ac86-63fdeb781a00/w=350" alt="WOMEN" />
                        @endif
                    </div>
                    <div class="tech_text two">
                        <h2>Women's Fashion</h2>
                        <p>
                            Elevate your wardrobe with our women's fashion collection,
                            where style meets comfort and versatility. From chic Dresses
                            and trendy Jumpsuits to cozy Hoodies and Sweatshirts, we offer
                            stylish options for every occasion.
                        </p>
                        <p>
                            Stay fashionable with our elegant Coats and Jackets or keep it
                            casual with Jeans, Joggers, and Leggings that promise a
                            perfect fit and comfort. Our Tops, T-Shirts, Shirts, Blouses,
                            and Polo Shirts offer endless possibilities for effortless
                            style.
                        </p>
                        <p>
                            Simplify your outfit choices with our coordinated Co-ords and
                            Multipacks, or stay cool in the warmer months with our Shorts
                            and Skirts. Complete your look with our versatile Trousers.
                        </p>
                    </div>
                </div>
                <div class="tech_body one">
                    <div class="tech_text">
                        <h2>Boy's Fashion</h2>
                        <p>
                            Explore our stylish and comfortable boys' fashion collection,
                            perfect for every adventure and occasion. From playful Co-ords
                            and Multipacks to cool Jeans and Joggers, we have outfits that
                            make dressing fun and easy.
                        </p>
                        <p>
                            Keep him cozy with our trendy Sweatshirts, Hoodies, and warm
                            Coats and Jackets, ideal for any season. Our collection also
                            includes classic Shirts, versatile T-Shirts, and Vests for
                            endless styling possibilities.
                        </p>
                        <p>
                            For active days, our comfortable Shorts and durable Trousers
                            are must-haves, while our snug Nightwear ensures a restful
                            night's sleep. Refresh his wardrobe with our high-quality
                            boys' fashion pieces designed to suit his unique style and
                            energy.
                        </p>
                    </div>

                    <div class="tech_img">
                        @if (Str::isMobile())
                            <img src="https://imagedelivery.net/z7kNRC7XttEX5-KXLRrx3A/1707396765-344307gYBR.JPG/w=205" alt="BOY" />
                        @else
                            <img src="https://imagedelivery.net/z7kNRC7XttEX5-KXLRrx3A/1707396765-344307gYBR.JPG/w=350" alt="BOY" />
                        @endif
                    </div>
                </div>
                <div class="tech_body two">
                    <div class="tech_img">
                        @if (Str::isMobile())
                            <img src="https://imagedelivery.net/z7kNRC7XttEX5-KXLRrx3A/1707800225-64315Lzayu.JPG/w=205" alt="GIRL" />
                        @else
                            <img src="https://imagedelivery.net/z7kNRC7XttEX5-KXLRrx3A/1707800225-64315Lzayu.JPG/w=350" alt="GIRL" />
                        @endif
                    </div>
                    <div class="tech_text two">
                        <h2>Girl's Fashion</h2>
                        <p>
                            Discover our enchanting girls' fashion collection, designed to
                            bring style and comfort to every occasion. From adorable
                            Dresses and Jumpsuits to playful Co-ords and Multipacks,
                            dressing up has never been more fun.
                        </p>
                        <p>
                            Our collection offers cozy Sweatshirts, Hoodies, and snug
                            Nightwear, perfect for keeping her warm and comfortable.
                            Explore trendy Jeans, Dungarees, Leggings, and Joggers for
                            versatile options that suit her active lifestyle.
                        </p>
                        <p>
                            Mix and match with our stylish Tops and T-Shirts for endless
                            outfit possibilities, or stay cool with our charming Shorts.
                            Complete her look with our fashionable Trousers, crafted for
                            both play and special occasions.
                        </p>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>
<!--  End Teck Pack Section-->
